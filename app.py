...
# Display the Tesseract path for debugging

pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'


# Check Tesseract installation path in the deployment environment


try:
    # On Windows, use 'where' instead of 'which'
    tesseract_path = subprocess.check_output(['where', 'tesseract']).decode('utf-8').strip()
    #st.write(f"Tesseract executable is located at: {tesseract_path}")
except subprocess.CalledProcessError:
    st.write("Tesseract is not installed or not found in the PATH.")

# Check Tesseract version
try:
    tesseract_version = subprocess.check_output(['tesseract', '--version']).decode('utf-8').strip()
   # st.write(f"Tesseract Version: {tesseract_version}")
except Exception as e:
    st.error(f"Error checking Tesseract version: {e}")

pytesseract.pytesseract.tesseract_cmd = tesseract_path  # Set it to the detected path from above
#st.write(f"Using Tesseract executable at: {pytesseract.pytesseract.tesseract_cmd}")

# Display the Tesseract path being used by pytesseract
logger.debug(f"Tesseract executable is located at: {tesseract_path}")
logger.debug(f"Using Tesseract executable at: {pytesseract.pytesseract.tesseract_cmd}")
logger.debug(f"Available Tesseract Languages: {pytesseract.get_languages(config='')}")



# Display installed Tesseract languages
try:
    available_languages = subprocess.check_output(['tesseract', '--list-langs']).decode('utf-8').strip()
    #st.write(f"Available Tesseract Languages: {available_languages}")
except Exception as e:
    st.error(f"Error checking Tesseract languages: {e}")

environment = os.getenv('ENVIRONMENT', 'production')  # Set 'development' in local environment

# Check Tesseract installation path (already found above)
tesseract_path = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
pytesseract.pytesseract.tesseract_cmd = tesseract_path

# Only show debug information if in development mode
if environment == 'development':
    st.write(f"Tesseract executable is located at: {tesseract_path}")
    st.write(f"Using Tesseract executable at: {pytesseract.pytesseract.tesseract_cmd}")
    st.write(f"Available Tesseract Languages: {pytesseract.get_languages(config='')}")
